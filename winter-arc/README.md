...
## Wdrożenie na VPS lukaszdobija.pl (nginx)

Strona główna działa jako usługa Next.js za nginx, więc panel wchodzi jako osobna
lokalizacja obsługiwana przez PHP-FPM. Baza poza katalogiem publicznym.

```bash
apt install php8.2-fpm php8.2-sqlite3          # raz, jeśli PHP nie ma na serwerze
mkdir -p /opt/lukaszdobija/arc /opt/lukaszdobija/arc-data
cp -r winter-arc/. /opt/lukaszdobija/arc/
chown -R www-data:www-data /opt/lukaszdobija/arc-data
```

W `/opt/lukaszdobija/arc/config.php`:
```php
'db_path' => '/opt/lukaszdobija/arc-data/winter-arc.sqlite',
```

W bloku `server` domeny (obok `location /` kierującego na Next.js):
```nginx
location /arc/ {
    alias /opt/lukaszdobija/arc/;
    index index.php;
    try_files $uri $uri/ /arc/index.php?$args;
    ...
    location ~ /arc/data/ { deny all; }
    location ~ \.(sqlite|sqlite-wal|sqlite-shm|md)$ { deny all; }
}
```

Po `nginx -t && systemctl reload nginx` panel jest pod `https://lukaszdobija.pl/arc/`.
Ten adres wpisz w panelu strony w zakładce **Aplikacje** jako skrót.
